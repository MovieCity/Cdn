(function(){
  ...
  // Add a CSS class for easy customization.
  container.classList.add('ruriorg-container');
  // Set computed width and height from config.
  ...
  // Minimal default inline styles (can be overridden by external CSS)
  ...
  video.controls = false; // custom controls will be built
  container.appendChild(video);

  // --- HLS Support ---
  // If the video file is HLS (ends with .m3u8) then use native support if available or load hls.js.
  if (config.file && config.file.match(/\.m3u8$/)) {
    if (video.canPlayType('application/vnd.apple.mpegurl')) {
      ...
      // Load hls.js dynamically if not already present.
  ...
  // Minimal default styles; these are easily overridden via external CSS.
  ...
  previewContainer.style.bottom = '50px'; // adjust as needed
  ...
  // Parse a time string (e.g., "00:00.000") into seconds.
  ...
  function parseVTT(vttText) {
    ...
    for (var i = 0; i < lines.length; i++) {
      ...
      if (line.indexOf('-->') !== -1) {
        ...
        // Expecting cue details, e.g., "x:0,y:0,w:160,h:90"
  ...
  // Expose a global API for customization.
  window.RuriORG = {
    container: container,
    video: video,
    controls: controls,
  ...
  // If jQuery is available, trigger a custom event for further customization.
  if (window.jQuery) {
