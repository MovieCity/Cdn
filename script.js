(function(){
  ...
  // Create the outer container using the responsive aspect ratio trick.
  var container = document.getElementById('ruriorg-container');
  if (!container) {
    container = document.createElement('div');
    container.id = 'ruriorg-container';
  ...
  if (config.aspectratio) {
    ...
    container.style.paddingBottom = "56.25%"; // Default for 16:9.
  }
  container.style.margin = "20px auto";
  container.style.backgroundColor = "#000";
  container.style.overflow = "hidden";

  // Create an inner wrapper that fills the container.
  var wrapper = document.createElement('div');
  wrapper.id = 'ruriorg-wrapper';
  ...
  // Create the HTML5 video element and append it to the wrapper.
  ...
  // The controls are appended to the wrapper so they remain within the 16:9 area.
  ...
  function parseVTT(vttText) {
    ...
    for (var i = 0; i < lines.length; i++) {
      ...
      if (!line) { if (cue) { cues.push(cue); cue = null; } continue; }
